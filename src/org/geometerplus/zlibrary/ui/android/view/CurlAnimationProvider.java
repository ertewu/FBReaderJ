...
class CurlAnimationProvider extends AnimationProvider {
    ...
    protected void drawInternal(Canvas canvas) {
        if (myUseCanvasHack) {
            ...
            // 1) for GLES20Canvas we got an UnsupportedOperationException in
            // clipPath
            // 2) View.setLayerType(LAYER_TYPE_SOFTWARE) does not work properly
            // in some cases
    ...
    private void drawInternalNoHack(Canvas canvas) {
        ...
        /*
         * canvas.save(); canvas.clipPath(myEdgePath); final Matrix m = new
         * Matrix(); m.postScale(1, -1); m.postTranslate(x - cornerX, y +
         * cornerY); final float angle; if (cornerY == 0) { angle = -180 /
         * 3.1416f * (float)Math.atan2(x - cornerX, y - y1); } else { angle =
         * 180 - 180 / 3.1416f * (float)Math.atan2(x - cornerX, y - y1); }
         * m.postRotate(angle, x, y); canvas.drawBitmap(fgBitmap, m,
         * myBackPaint); canvas.restore();
         */
    }
