...
public final class ZLTextParagraphCursor {
    ...
    private static final class Processor {
        ...
        private final int myFirstMark;
        private int myLastMark;
        private final List<ZLTextMark> myMarks;

        private Processor(ZLTextView view, ZLTextParagraph paragraph, LineBreaker lineBreaker, List<ZLTextMark> marks, int paragraphIndex,
        ...
        void fill() {
            ...
            for (ZLTextParagraph.EntryIterator it = myParagraph.iterator(); it.next();) {
                ...
                case ZLTextParagraph.Entry.HYPERLINK_CONTROL: {
                    ...
                    if (hyperlinkType != 0) {
                        final ZLTextHyperlinkControlElement control = new ZLTextHyperlinkControlElement(it.getControlKind(), hyperlinkType,
                                it.getHyperlinkId());
    ...
    ZLTextParagraphCursor(ZLTextView view, ZLTextModel model, int index) {
        View = view;
        Model = model;
        //index是最后一个章节的index啊 ，以这个index取，也就是一个文章的最后一章啦
