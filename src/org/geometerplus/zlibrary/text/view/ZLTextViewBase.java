...
abstract class ZLTextViewBase extends ZLView {
    ...
    public int getTextColumnWidth() {
        return twoColumnView() ? (getContextWidth() - getLeftMargin() - getSpaceBetweenColumns() - getRightMargin()) / 2 : getContextWidth()
    ...
    final void setTextStyle(ZLTextStyle style) {
        ...
        getContext().setFont(style.getFontEntries(), style.getFontSize(metrics()), style.isBold(), style.isItalic(), style.isUnderline(),
    ...
    boolean isStyleChangeElement(ZLTextElement element) {
        return element == ZLTextElement.StyleClose || element instanceof ZLTextStyleElement || element instanceof ZLTextControlElement;
    }
    ...
    final int getElementHeight(ZLTextElement element) {
        if (element instanceof ZLTextWord || element instanceof ZLTextFixedHSpaceElement) {
    ...
    private final void drawString(ZLPaintContext context, int x, int y, char[] str, int offset, int length, ZLTextWord.Mark mark, ZLColor color,
            int shift) {
        if (mark == null) {
